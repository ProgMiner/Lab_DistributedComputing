...
#define MUTEX_PENDING_SIZE (MAX_PROCESS_ID + 1)
...
struct ipc {
    ...
    bool mutex_pending[MUTEX_PENDING_SIZE];
    timestamp_t my_await_time;
...
static bool send_reply(struct ipc * ipc, local_id id) {
    Message reply;
...
static bool handle_next_message(struct ipc * ipc) {
    ...
    switch (msg.s_header.s_type) {
        ...
        case CS_REQUEST:
            switch (ipc->mutex_state) {
                case MUTEX_STATE_OUTSIDE:
                    if (!send_reply(ipc, src)) {
                        return false;
                    }

                    break;

                case MUTEX_STATE_AWAIT:
                    if (ipc->my_await_time > msg.s_header.s_local_time || (ipc->my_await_time == msg.s_header.s_local_time && ipc->id > src)) {
                        if (!send_reply(ipc, src)) {
                            return false;
                        }
                    } else {
                        ipc->mutex_pending[src] = true;
                    }

                    break;

                case MUTEX_STATE_INSIDE:
                    ipc->mutex_pending[src] = true;
                    break;
            }
    ...
    for (; i < n; ++i) {
        ...
        if (pid == 0) {
            struct ipc child_ipc = {
                ...
                .id = (local_id) i,
                .use_mutex = use_mutex,
                .mutex_state = MUTEX_STATE_OUTSIDE,
                .events_log = events_log_file,
                .context = { 0 },
            };

            memset(child_ipc.mutex_pending, 0, sizeof(child_ipc.mutex_pending));
...
int request_cs(const void * self) {
    ...
    ipc->mutex_state = MUTEX_STATE_AWAIT;
    ipc->my_await_time = get_lamport_time();

    // N - parent - me
    const local_id needed_replies = (local_id) (ipc->n - 2);
    while (ipc->context.replies < needed_replies) {
        if (!handle_next_message(ipc)) {
            return -1;
        }
    }

    ipc->mutex_state = MUTEX_STATE_INSIDE;
    return 0;
}
...
int release_cs(const void * self) {
    ...
    if (ipc->mutex_state != MUTEX_STATE_INSIDE) {
        return 0;
    }

    for (local_id id = 0; id < ipc->n; ++id) {
        if (ipc->mutex_pending[id]) {
            Message msg;
            ...
            const int ret = send(ipc, id, &msg);
            if (ret < 0) {
                return ret;
            }

            ipc->mutex_pending[id] = false;
        }
    }

    ipc->mutex_state = MUTEX_STATE_OUTSIDE;
    return 0;
}
